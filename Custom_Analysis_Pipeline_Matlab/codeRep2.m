...
filename={'A_0_Red_GenExprMat.txt','B_0_Red_GenExprMat.txt','Human_Embryo_fulldataset_week_6.txt'};
filename={['A_' num2str(day_comp) '_Red_GenExprMat.txt'],...
    ['B_' num2str(day_comp) '_Red_GenExprMat.txt'],...
    'Human_Embryo_fulldataset_week_6.txt'};
...
aux_mat=[all_expr_mat_ord{1,groups_use(1)},all_expr_mat_ord{1,groups_use(2)}];
aux_mat_sum=sum(aux_mat,2);
aux_mat=aux_mat((aux_mat_sum>100),:);
dist_mat=squareform(pdist(zscore(aux_mat)','correlation'));
%imagesc(1-dist_mat(1:500,501:end))
imagesc(1-dist_mat)
...
figure;imagesc(dist_mat(1:length(all_expr_mat_ord{4,groups_use(1)}),...
    (length(all_expr_mat_ord{4,groups_use(1)})+1):end));
xticks(1:length(all_expr_mat_ord{4,groups_use(2)}))
xticklabels(all_expr_mat_ord{4,groups_use(2)})
yticklabels(phenos(all_expr_mat_ord{4,groups_use(1)}))
...
figure;imagesc(corr_mat_selgenes);
xticks(1:length(all_expr_mat_ord{4,groups_use(2)}))
xticklabels(all_expr_mat_ord{4,groups_use(2)})
yticklabels(phenos(all_expr_mat_ord{4,groups_use(1)}))
title('Cumulative corr Selected genes')
colorbar

figure;imagesc(mutinfo_selgenes);
xticks(1:length(all_expr_mat_ord{4,groups_use(2)}))
xticklabels(all_expr_mat_ord{4,groups_use(2)})
yticklabels(phenos(all_expr_mat_ord{4,groups_use(1)}))
title('MutInfo Selected genes')
colorbar
