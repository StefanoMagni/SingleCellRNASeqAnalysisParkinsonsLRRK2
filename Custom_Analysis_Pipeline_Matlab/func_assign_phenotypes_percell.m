...
function [both_cum_expression_all_phenos,all_genes_to_check,index_group_genes,ind_max_pheno]=...
    ...
    colors,title_eachfig,az,el,obj,flag_save,all_lims,name_save,labels,alpha_val,colormap_val,...
...
if 1
    ...
    data_group_expr_all=all_expr{1,3};
    data_group_exprs_A=data_group_expr_all(:,labels==0);
    ...
    writetable(data_group_exprs_A,['A_' num2str(day_comp) '_Red_GenExprMat.csv'],'WriteRowNames',1)
    
    data_group_expr_B=data_group_expr_all(:,labels==1);
    ...
    writetable(data_group_expr_B,['B_' num2str(day_comp) '_Red_GenExprMat.csv'],'WriteRowNames',1)
...
if flag_plot
    ...
    view(az,el)
    plot_gaussians(data_group,obj,indexes_clusters,h1,num_dim_tsne,alpha_val,colormap_val);
